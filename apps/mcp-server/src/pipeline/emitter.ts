...
export function emitPolicyCrate(spec: PolicySpec, outputDir?: string): EmittedCrate {
  ...
  const files: Record<string, string> = {
    "Cargo.toml": renderCargoToml(crateName, layerDescriptions),
    ".gitignore": "target/\nCargo.lock\n",
    "src/lib.rs": isGenerate
      ? renderGenerateLib(structName, cap, periodLedgers, assetId, spec.rationale)
...
function renderCargoToml(crateName: string, description: string): string {
  return `[package]
name = "${crateName}"
version = "0.1.0"
edition = "2021"
...
impl Policy for ${structName} {
    ...
    fn install(e: &Env, install_params: InstallParams, context_rule: ContextRule, smart_account: Address) {
        ...
        let pk = StorageKey::Params(smart_account.clone(), context_rule.id);
        let sk = StorageKey::State(smart_account.clone(), context_rule.id);
        e.storage().persistent().set(&pk, &install_params);
        e.storage().persistent().extend_ttl(&pk, TTL_THRESHOLD, EXTEND_AMOUNT);
        e.storage().persistent().set(&sk, &SpendState { spent: 0, window_start_ledger: e.ledger().sequence() });
        e.storage().persistent().extend_ttl(&sk, TTL_THRESHOLD, EXTEND_AMOUNT);
    ...
    fn uninstall(e: &Env, context_rule: ContextRule, smart_account: Address) {
        smart_account.require_auth();
        let pk = StorageKey::Params(smart_account.clone(), context_rule.id);
        if !e.storage().persistent().has(&pk) { panic!("${structName}: not installed"); }
        e.storage().persistent().remove(&pk);
